...
tbApp.controller('taskboardController', function($scope, $filter, $http) {
	...
	$scope.owners = [ '<All Owners>', '<No Owner>' ];
	...
	$scope.taskFolders = [ { type: 0 }, { type: 1 }, { type: 2 }, { type: 3 }, { type: 4 }, { type: 5 } ];
	...
	$scope.init = function() {
		...
		$scope.$watchGroup([ 'filter.search', 'filter.private', 'filter.category', 'filter.owner' ], function(
			newValues,
			oldValues
		) {
				...
				try {
					...
					if (ui.item.sortable.droptarget) {
						...
						if (
							...
							writeLog(
								'Drag and drop canceled because of limit reached. From ' +
									e.target.id +
									' to ' +
									ui.item.sortable.droptarget.attr('id')
							);
							...
							switch (ui.item.sortable.droptarget[0].id) {
								case 'folder-' + SOMEDAY:
									var tasksfolder = getTaskFolder(
										...
										$scope.config.TASKFOLDER.STORES,
										$scope.config.TASKFOLDER.FOLDER
									);
	...
	var getTasksFromOutlook = function(path, sort, folderStatus) {
		try {
			var i,
				array = [];
			var tasks = getTaskItems(path, $scope.config.TASKFOLDER.STORES, $scope.config.TASKFOLDER.FOLDER);
			...
			for (i = 1; i <= count; i++) {
				...
				if (task.Status == folderStatus || folderStatus == -1) {
					array.push({
						...
						owner: getTaskOwner(task.Recipients),
	...
	$scope.initTasks = function() {
		try {
			...
			writeLog("$scope.filter.owner = " + $scope.filter.owner);
			$scope.applyFilters();
			...
			if (true) {
				...
				for (i = 0; i < count; i++) {
					if (tasks[i].status != $scope.config.STATUS.NOT_STARTED.TEXT) {
						...
						//taskitem.Move(getTaskFolder($scope.taskFolders[SPRINT].name, $scope.config.TASKFOLDER.STORES, $scope.config.TASKFOLDER.FOLDER));
						movedTask = true;
					}
				}
				if (movedTask) {
					...
					$scope.taskFolders.forEach(function(taskFolder) {
						taskFolder.tasks = getTasksFromOutlook(
							taskFolder.name,
							taskFolder.sort,
							taskFolder.initialStatus
						);
						taskFolder.filteredTasks = taskFolder.tasks;
					});
				}
			...
			if (true) {
				...
				for (i = 0; i < count; i++) {
					if (tasks[i].startdate.getFullYear() != 4501) {
						...
						if (seconds >= 0) {
							var taskitem = getTaskItem(tasks[i].entryID);
							//taskitem.Move(getTaskFolder($scope.taskFolders[BACKLOG].name, $scope.config.TASKFOLDER.STORES, $scope.config.TASKFOLDER.FOLDER));
							movedTask = true;
						}
					}
				}
	...
	$scope.sendFeedback = function(includeConfig, includeState, includeLog) {
		try {
			...
			mailItem.Subject =
				'JanBan version ' + $scope.version + ' Feedback (Outlook version: ' + getOutlookVersion() + ')';
		...
		try {
			...
			if ($scope.config.COMPLETED_FOLDER.REPORT.DISPLAY) {
				...
				for (i = 1; i <= count; i++) {
					...
					if (rptBodyNotes(tasks(i).Body, 10000)) {
						mailBody += '<br>' + '<font color=gray>' + rptBodyNotes(tasks(i).Body, 10000) + '</font>';
					}
					mailBody += '</li>';
				}
				mailBody += '</ul>';
	...
	var getTaskOwner = function(Recipients) {
		try {
			var retval = '';
			if (Recipients.Count > 0) {
				retval = Recipients(1).Name;
				if ($.inArray(retval, $scope.owners) === -1) {
					$scope.owners.push(retval);
					$scope.owners = $scope.owners.sort();
				}
			}
			return retval;
		} catch (error) {
			writeLog('getTaskOwner: ' + error);
		}
	};

	var rptBodyNotes = function(str, limit) {
		try {
			// remove empty lines, cut off text if length > limit
			//str = str.replace(/^(?=\n)$|^\s*|\s*$|\n\n+/gm, '');
	...
	var taskStatusText = function(status) {
		try {
			...
			if (status == $scope.config.STATUS.DEFERRED.VALUE) {
				return $scope.config.STATUS.DEFERRED.TEXT;
			}
			return '';
		} catch (error) {
	...
	var applyConfig = function() {
		try {
			...
			$scope.taskFolders[BACKLOG].initialStatus = $scope.config.STATUS.DEFERRED.VALUE;
	...
	const DEFAULT_CONFIG = {
		...
		LOG_ERRORS: false,
		TASKFOLDER: {
			STORES: 'SharePoint Lists',
			FOLDER: 'AU Sales Engineers - Team Tasks',
			URL:
				'https://intersystemscorporation.sharepoint.com/sites/AUSalesEngineers/Lists/Test%20Team%20Tasks/List.aspx'
		}
	};

	var getState = function() {
		try {
			var state = { private: '0', search: '', category: '<All Categories>', owner: '<All Owners>' }; // default state
			...
			if (state.owner === undefined) state.owner = '<All Owners>';
...
*/
			...
			if (
				typeof $scope.config.INPROGRESS_FOLDER.LIMIT === 'string' ||
				$scope.config.INPROGRESS_FOLDER.LIMIT instanceof String
			) {
				$scope.config.INPROGRESS_FOLDER.LIMIT = parseInt($scope.config.INPROGRESS_FOLDER.LIMIT);
				isChanged = true;
			}
			if (
				typeof $scope.config.NEXT_FOLDER.LIMIT === 'string' ||
				$scope.config.NEXT_FOLDER.LIMIT instanceof String
			) {
			...
			var kanbanFolder = getTaskFolder(
				'Kanban',
				$scope.config.TASKFOLDER.STORES,
				$scope.config.TASKFOLDER.FOLDER
			);
	...
	var setUrls = function() {
		...
		$scope.VERSION_URL = 'https://janware.nl/gitlab/version.txt';
		$scope.DOWNLOAD_URL = 'https://janware.nl/gitlab/janban.zip';
		$scope.WHATSNEW_URL = 'https://janware.nl/gitlab/whatsnew.html';
		$scope.PING_URL =
			'https://janvanveldhuizen.outsystemscloud.com//JanbanApi/rest/janbanping/hello?email={{email}}&name={{name}}&version={{version}}';
	...
	var getVersion = function() {
		try {
			$http
				.get($scope.VERSION_URL, { headers: { 'Cache-Control': 'no-cache', Pragma: 'no-cache' } })
